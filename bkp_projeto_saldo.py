...
def preenche_coluna_mes_ano():
    ...
    cursor.execute("""UPDATE entradas_3c
                    SET ANO = SUBSTR("Data Lançamento", -6)""")
...
def exclui_dados_entradas():
    ...
    try:
        cursor.execute((
                           f"DELETE FROM ENTRADAS_3C WHERE Centro = {entrada_filial} AND PERÍODO = {entrada_periodo} AND ANO = {entrada_ano}"))
        cursor_consolidado.execute((
                                       f"DELETE FROM ENTRADAS_3C WHERE Centro = {entrada_filial} AND PERÍODO = {entrada_periodo} AND ANO = {entrada_ano}"))
        cxn.commit()
        cxn_consolidado.commit()
        logging.info(
            f'Arquivo de entrada da filial {entrada_filial} do período {entrada_periodo} e do ano {entrada_ano} foi excluido do sistema')
...
def exportar_saldo_atual():
    ...
    try:
        ...
        df2 = pd.read_sql("SELECT EMPRESA, 'Saldo Qtd', 'ICMS ST Total Atualizado' FROM SALDO_ATUAL GROUP BY EMPRESA",
                          cxn_consolidado)
        df2.to_excel(writer, index=False, sheet_name="saldo_atual_por_empresa")
        df3 = pd.read_sql(
            "SELECT EMPRESA,Centro, 'Saldo Qtd', 'ICMS ST Total Atualizado' FROM SALDO_ATUAL GROUP BY Centro, EMPRESA",
            cxn_consolidado)
...
def transforma_dados():
    ...
    novo_cabecalho = ["CodigoCenario", "Data", "Material", "TipoAvaliacao", "Docnum", "Empresa", "CodigoCentro",
                      "Divisao", "ValorICMS", "ValorICMSST", "ValorIPI"]
...
def mensagem_erro():
    messagebox.showinfo("Erro!", "Ação não executada. Contate o Administrador")
